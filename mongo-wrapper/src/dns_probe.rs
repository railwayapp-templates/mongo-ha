...
//! decisions.
//!
//! The private-network resolver answers each query with a live lookup and
//! returns:
//!
//! - **NXDOMAIN** when the name has zero live container IPs registered —
//!   a deleted service, or a shorter no-container window such as
//!   mid-redeploy.
//! - **SERVFAIL** when the resolver's own backing lookup cannot complete.
//!
//! A partition therefore can never manufacture NXDOMAIN: an unreachable
//! peer's records are still served (the resolver's view does not depend on
//! the querying host's data-plane connectivity), and a host cut off from
//! the resolver gets SERVFAIL. That makes a *persistent*
...
//! right now" — exactly what the bootstrap guard needs before it may stop
//! waiting on a scaled-away peer (see `crate::rs`). Transient NXDOMAIN
//! windows (a redeploy) are filtered by the caller requiring the verdict
//! continuously across its whole dwell.
//!
//! (Ported verbatim from redis-ha's dns_probe, where the same verdict arms
//! the Sentinel membership prune.)
//!
...
//!
//! The query asks for AAAA (the private network is IPv6-only); the
...
pub enum ProbeDetail {
    /// The peer is declared as a raw IP literal — never provable as deleted.
    IpLiteral,
...
pub async fn probe_name_detailed(name: &str, deadline: Duration) -> (NameVerdict, ProbeDetail) {
    ...
    // proof. Guards against a RS_SEEDS entry declared by raw address.
...
mod tests {
    ...
    fn encode_builds_a_wellformed_question() {
        let packet = encode_query(0xBEEF, "mongo-2.railway.internal.").unwrap();
        assert_eq!(&packet[0..2], &[0xBE, 0xEF]);
        assert_eq!(packet[2], 0x01); // RD
        assert_eq!(&packet[4..6], &[0x00, 0x01]); // one question
                                                  // QNAME: 7"mongo-2" 7"railway" 8"internal" 0
        let qname_start = 12;
        assert_eq!(packet[qname_start], 7);
        assert_eq!(&packet[qname_start + 1..qname_start + 8], b"mongo-2");
        let end = packet.len();
