//! Volume-lifetime runtime lock: at most one MongoDB container touches this
//! volume at a time.
//!
//! A redeploy (or an instance restart during host maintenance) can leave the
//! old and new containers briefly overlapping on the shared volume.
//! WiredTiger's own `mongod.lock` makes the second mongod ABORT in that state
//! — a crash-loop whose shape depends on which process wins the race. The
...
//! container waits for the previous holder's supervisor to exit before
//! anything (the keyfile, mongod itself) touches the data directory, and
//! fail-stops loudly on timeout so the restart policy retries the boot.
//!
...
//! outlives mongod by construction (`process_manager::supervise` reaps it
//! before exiting), which is what makes its lifetime a faithful proxy for
//! "this container is done with the volume".
//!
//! (Ported from mysql-ha / redis-ha, where the same guard closes the same
//! overlap window.)
...
pub(crate) const RUNTIME_LOCK_FILE: &str = ".railway-mongo-runtime.lock";
                ...
                if Instant::now() >= deadline {
                    bail!(
                        "previous container did not release the volume within {wait_secs}s; \
                         refusing to start mongod against a volume another instance may still be using"
                    );
                }
                file = returned;
...
mod tests {
    ...
    fn acquires_when_uncontended_and_blocks_reacquisition() {
        let dir = std::env::temp_dir().join(format!("mongo-rt-lock-test-{}", std::process::id()));
    ...
    fn times_out_when_another_holder_keeps_the_lock() {
        let dir =
            std::env::temp_dir().join(format!("mongo-rt-lock-timeout-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let dir_str = dir.to_str().unwrap();
    ...
    fn missing_directory_fails_open() {
        let dir =
            std::env::temp_dir().join(format!("mongo-rt-lock-missing-{}", std::process::id()));
