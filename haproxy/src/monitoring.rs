...
pub fn run_monitoring_loop(mut child: Child, telemetry: &Telemetry) -> Result<()> {
    ...
    loop {
        ...
        match check_primary_health(&client) {
            ...
            Ok(false) => {
                if !no_primary_alerted {
                    warn!("no healthy primary backend — cluster has no writable primary");
                    telemetry.send(TelemetryEvent::NoPrimary {
                        backends: vec!["mongo_primary_backend".to_string()],
                    });
...
/// Returns true if at least one server in mongo_primary_backend is UP.
fn check_primary_health(client: &Client) -> Result<bool> {
    ...
    for line in text.lines() {
        ...
        let svname = cols[1];
        let status = cols[17];
        if pxname == "mongo_primary_backend" && svname != "BACKEND" && status == "UP" {
            return Ok(true);
        }
    }
    Ok(false)
