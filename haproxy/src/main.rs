...
fn main() -> Result<()> {
    ...
    let telemetry = Telemetry::from_env("mongo-ha");
    let config = Config::from_env()?;
    let nodes = parse_nodes(&config.mongo_nodes)?;

    info!(
        nodes = %config.mongo_nodes,
        count = nodes.len(),
    ...
    if let Some(auth) = &config.stats_auth {
        ...
        info!("stats page: loopback only (no HAPROXY_STATS_PASSWORD / MONGOPASSWORD)");
    }
