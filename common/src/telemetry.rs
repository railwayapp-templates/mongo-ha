...
/// reused verbatim across postgres-ha, redis-ha, mysql-ha and mongo-ha and the wire
...
impl TelemetryEvent {
    ...
    pub fn event_type(&self) -> &'static str {
        match self {
            Self::NodeStarted { .. } => "mongo_ha.node_started",
            Self::RoleChanged { .. } => "mongo_ha.role_changed",
            Self::ProcessDied { .. } => "mongo_ha.process_died",
            Self::NoPrimary { .. } => "mongo_ha.no_primary",
            Self::ComponentError { .. } => "mongo_ha.component_error",
        }
    }
...
mod tests {
        ...
        for event in all_events() {
            ...
            for field in ["command", "error", "stacktrace"] {
                let value = input[field]
                    .as_str()
                    .unwrap_or_else(|| panic!("{field} missing for {:?}", event.event_type()));
                assert!(
                    !value.is_empty(),
                    "{field} empty for {}",
                    event.event_type()
            ...
            assert_eq!(input["version"], "mongo-wrapper");
        }
    }
    ...
    fn event_types_are_distinct_and_namespaced() {
        ...
        for event in all_events() {
            ...
            assert!(ty.starts_with("mongo_ha."), "{ty} is not namespaced");
    ...
    fn stacktrace_carries_the_structured_event() {
        ...
        let payload = telemetry.build_payload(&event);
        let raw = payload["variables"]["input"]["stacktrace"]
            .as_str()
            .unwrap();
